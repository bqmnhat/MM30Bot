...
public class Strategy2 extends BaseStrategy {
    ...
    public Map<String, Double> steerInput(Map<String, Plane> planes) {
        ...
        for (Plane p : teamPlanes) {
            Double steer = getSteerForPlane(p, opponents, teamPlanes);
    ...
    private double evaluatePlaneState(Plane plane, List<Plane> opponents) {
        ...
        for (Plane opponent : opponents) {
            ...
            if (distance < plane.getStats().getAttackRange()) {
                score += 50; // Reward being in range to attack
            } else {
                score -= distance; // Penalize being too far
            }
        }
